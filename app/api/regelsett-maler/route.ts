...
export async function GET() {
  ...
  try {
    ...
    const regelsettMaler = result.records.map((record) => record.get('regelsettMal'));
    ...
    console.error('Feil ved henting av regelsett-maler:', error);
    return NextResponse.json(
      {
        success: false,
        error: 'Kunne ikke hente regelsett-maler',
      },
      { status: 500 }
    );
  } finally {
